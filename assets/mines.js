...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-163b4de3";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-163b4de3">'+'<span class="tile-face-163b4de3 tile-front-163b4de3"></span>'+'<span class="tile-face-163b4de3 tile-back-163b4de3"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-163b4de3")){return;}
var back=tile.querySelector(".tile-back-163b4de3");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-163b4de3");if(isMine){tile.classList.add("boom-163b4de3");}
if(dim){tile.classList.add("dimmed-163b4de3");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-163b4de3")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-163b4de3");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-163b4de3")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-163b4de3");void panelEl.offsetWidth;panelEl.classList.add("jolt-163b4de3");}
endRound();}
